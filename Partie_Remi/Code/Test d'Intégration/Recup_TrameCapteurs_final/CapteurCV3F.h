...
class CapteurCV3F {
	private:
		...
		void setTrameNMEA();											
		bool validationTrameNMEA_MWV();           		// Fonction permettant de savoir si la trame NMEA_MWV est correct (true or false)
	public:
		...
		const char* const getTrameNMEA_MWV();         	// Fonction permettant de recuperer la Trame NMEA083 MWV
		const char* const getTrameNMEA_XDR();         	// Fonction permettant de recuperer la Trame NMEA083 XDR
};
